...
def Logistic_Regression(Xtrain,Ytrain,Xtest,Ytest,alpha,theta,num_iters):
	...
	for i in range(num_iters):
		...
		scatter(iteration_no,err1,color='blue',marker='.',s=5,label="training Error")
		scatter(iteration_no,err2,color='green',marker='.',s =1,label="testing Error")
...
fp= open("/content/drive/My Drive/colab/test.txt","r")
st=fp.readlines()
length=len(st)
lt=[]
...
for i in range(len(Xtrain)):
	if(Ytrain[i]==1):
		digit_1_trlt_f1.append(Xtrain[i][1])
		digit_1_trlt_f2.append(Xtrain[i][2])
		# plt.scatter(Xtrain[i][1],Xtrain[i][2],color = 'green',marker='.',label="digit-1")
	else:
		digit_4_trlt_f1.append(Xtrain[i][1])
		digit_4_trlt_f2.append(Xtrain[i][2])
		# plt.scatter(Xtrain[i][1],Xtrain[i][2],color = 'blue',marker='.',label="digit-4")
plt.scatter(digit_1_trlt_f1,digit_1_trlt_f2,color = 'green',marker='.',label="digit-1")
plt.scatter(digit_4_trlt_f1,digit_4_trlt_f2,color = 'blue',marker='.',label="digit-4")
plt.xlabel("avgBrightness")
plt.ylabel("avgSymmentry")
plt.legend(loc='upper right')
f2=figure(2)
xlim(0,10000)
ylim(0,2)
